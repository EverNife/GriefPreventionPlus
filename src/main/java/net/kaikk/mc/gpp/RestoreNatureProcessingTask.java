...
class RestoreNatureProcessingTask implements Runnable {
	...
	private final Player player; // absolutely must not be accessed. not thread
	// safe.
	...
	private final ArrayList<Integer> notAllowedToHang; // natural blocks which
	// don't naturally hang
	// in their air
	private final ArrayList<Integer> playerBlocks; // a "complete" list of
	...
	public void run() {
		...
		final RestoreNatureExecutionTask task = new RestoreNatureExecutionTask(this.snapshots, this.miny, this.lesserBoundaryCorner, this.greaterBoundaryCorner, this.player);
	...
	private void coverSurfaceStone() {
		for (int x = 1; x < (this.snapshots.length - 1); x++) {
			for (int z = 1; z < (this.snapshots[0][0].length - 1); z++) {
				final int y = this.highestY(x, z, true);
	...
	private void fillBigHoles() {
		for (int x = 1; x < (this.snapshots.length - 1); x++) {
			for (int z = 1; z < (this.snapshots[0][0].length - 1); z++) {
				...
				// do the same for one layer beneath that (because a future
				// restoration step may convert surface stone to sand, which
				// falls down)
	...
	private void fillHolesAndTrenches() {
		final ArrayList<Integer> fillableBlocks = new ArrayList<Integer>();
		...
		final ArrayList<Integer> notSuitableForFillBlocks = new ArrayList<Integer>();
			...
			for (int x = 1; x < (this.snapshots.length - 1); x++) {
				for (int z = 1; z < (this.snapshots[0][0].length - 1); z++) {
					for (int y = 0; y < (this.snapshots[0].length - 1); y++) {
						...
						final BlockSnapshot leftBlock = this.snapshots[x + 1][y][z];
						final BlockSnapshot rightBlock = this.snapshots[x - 1][y][z];
						...
						final BlockSnapshot upBlock = this.snapshots[x][y][z + 1];
						final BlockSnapshot downBlock = this.snapshots[x][y][z - 1];
		...
		for (int x = 1; x < (this.snapshots.length - 1); x++) {
			for (int z = 1; z < (this.snapshots[0][0].length - 1); z++) {
				for (int y = miny; y < (this.snapshots[0].length - 1); y++) {
					final BlockSnapshot block = this.snapshots[x][y][z];
					final BlockSnapshot underBlock = this.snapshots[x][y][z];
				...
				for (int x = 1; x < (this.snapshots.length - 1); x++) {
					for (int z = 1; z < (this.snapshots[0][0].length - 1); z++) {
						...
						if ((block.typeId == Material.AIR.getId()) || ((block.typeId == Material.STATIONARY_WATER.getId()) && (block.data != 0))) {
							...
							final BlockSnapshot upBlock = this.snapshots[x][y][z + 1];
							final BlockSnapshot downBlock = this.snapshots[x][y][z - 1];
							final BlockSnapshot underBlock = this.snapshots[x][y - 1][z];
	...
	private int highestY(int x, int z, boolean ignoreLeaves) {
		...
		for (y = this.snapshots[0].length - 1; y > 0; y--) {
			final BlockSnapshot block = this.snapshots[x][y][z];
			if ((block.typeId != Material.AIR.getId()) && !(ignoreLeaves && (block.typeId == Material.SNOW.getId())) && !(ignoreLeaves && (block.typeId == Material.LEAVES.getId())) && !(block.typeId == Material.STATIONARY_WATER.getId()) && !(block.typeId == Material.WATER.getId()) && !(block.typeId == Material.LAVA.getId()) && !(block.typeId == Material.STATIONARY_LAVA.getId())) {
				return y;
			}
		}
	...
	private void reduceLogs() {
		final boolean jungleBiome = (this.biome == Biome.JUNGLE) || (this.biome == Biome.JUNGLE_HILLS);
		...
		for (int x = 1; x < (this.snapshots.length - 1); x++) {
			for (int z = 1; z < (this.snapshots[0][0].length - 1); z++) {
				...
				while ((thisy > (this.seaLevel - 1)) && ((this.snapshots[x][thisy][z].typeId == Material.STONE.getId()) || (this.snapshots[x][thisy][z].typeId == Material.SANDSTONE.getId()))) {
					final BlockSnapshot leftBlock = this.snapshots[x + 1][thisy][z];
					final BlockSnapshot rightBlock = this.snapshots[x - 1][thisy][z];
					final BlockSnapshot upBlock = this.snapshots[x][thisy][z + 1];
					final BlockSnapshot downBlock = this.snapshots[x][thisy][z - 1];
		...
		for (int x = 1; x < (this.snapshots.length - 1); x++) {
			for (int z = 1; z < (this.snapshots[0][0].length - 1); z++) {
				for (int y = this.seaLevel - 1; y < (this.snapshots[0].length - 1); y++) {
					final BlockSnapshot block = this.snapshots[x][y][z];
					if ((block.typeId == Material.STATIONARY_WATER.getId()) || (block.typeId == Material.STATIONARY_LAVA.getId()) || (block.typeId == Material.WATER.getId()) || (block.typeId == Material.LAVA.getId())) {
						block.typeId = Material.AIR.getId();
					}
				}
			}
		...
		for (int x = 1; x < (this.snapshots.length - 1); x++) {
			for (int z = 1; z < (this.snapshots[0][0].length - 1); z++) {
				for (int y = this.snapshots[0].length - 2; y > this.miny; y--) {
					...
					final BlockSnapshot downBlock = this.snapshots[x][y][z - 1];
					final BlockSnapshot underBlock = this.snapshots[x][y - 1][z];
					final BlockSnapshot aboveBlock = this.snapshots[x][y + 1][z];
					...
					// count adjacent non-air/non-leaf blocks
					if ((leftBlock.typeId == Material.SAND.getId()) || (rightBlock.typeId == Material.SAND.getId()) || (upBlock.typeId == Material.SAND.getId()) || (downBlock.typeId == Material.SAND.getId()) || (aboveBlock.typeId == Material.SAND.getId()) || (underBlock.typeId == Material.SAND.getId())) {
						this.snapshots[x][y][z].typeId = Material.SAND.getId();
					} else {
						this.snapshots[x][y][z].typeId = Material.AIR.getId();
					}
	...
	private void removeWallsAndTowers() {
		final int[] excludedBlocksArray = new int[] { Material.CACTUS.getId(), Material.LONG_GRASS.getId(), Material.RED_MUSHROOM.getId(), Material.BROWN_MUSHROOM.getId(), Material.DEAD_BUSH.getId(), Material.SAPLING.getId(), Material.YELLOW_FLOWER.getId(), Material.RED_ROSE.getId(), Material.SUGAR_CANE_BLOCK.getId(), Material.VINE.getId(), Material.PUMPKIN.getId(), Material.WATER_LILY.getId(), Material.LEAVES.getId() };

		final ArrayList<Integer> excludedBlocks = new ArrayList<Integer>();
		...
		do {
			...
			for (int x = 1; x < (this.snapshots.length - 1); x++) {
				for (int z = 1; z < (this.snapshots[0][0].length - 1); z++) {
					...
					final int righty = this.highestY(x + 1, z, false);
					final int lefty = this.highestY(x - 1, z, false);
					while ((lefty < thisy) && (righty < thisy)) {
						this.snapshots[x][thisy--][z].typeId = Material.AIR.getId();
						changed = true;
					}

					final int upy = this.highestY(x, z + 1, false);
					final int downy = this.highestY(x, z - 1, false);
	...
	static ArrayList<Integer> getPlayerBlocks(Environment environment, Biome biome) {
		...
		final ArrayList<Integer> playerBlocks = new ArrayList<Integer>();
		...
		// these are unnatural in the standard and nether worlds, but not in the
		// end
